...
import { CreditCard, LogOut, Users, UserPlus, History, DollarSign } from 'lucide-react';

type Page = 'accounts' | 'transactions' | 'create';
...
interface ManagerDashboardProps {
  ...
  onLogout: () => void;
  onCreateAccount: (name: string, username: string, password: string, initialBalance: number) => boolean;
  onUpdateAccount: (accountId: string, name: string, balance: number) => void;
...
export function ManagerDashboard({
  ...
  const handleCreateAccount = (e: React.FormEvent) => {
    ...
    const initialBalance = parseFloat(newInitialBalance) || 0;
    const success = onCreateAccount(newName, newUsername, newPassword, initialBalance);
  ...
  const handleEditAccount = () => {
    if (selectedAccount) {
      ...
      onUpdateAccount(selectedAccount.id, editName, balance);
      setIsEditing(false);
      setSelectedAccount({ ...selectedAccount, name: editName, balance });
    }
  };
                ...
                <div className="space-y-3 max-h-[600px] overflow-y-auto">
                  {userAccounts.map(account => (
                    <div
                      ...
                      className={`bg-gray-800 border-2 ${selectedAccount?.id === account.id ? 'border-blue-500' : 'border-gray-700'} rounded-lg p-4 cursor-pointer hover:border-blue-400 transition-colors`}
                    >
              ...
              <Card className="bg-gray-800 border-gray-700">
                ...
                <CardContent>
                  <form onSubmit={handleCreateAccount} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label className="text-gray-300">Initial Balance (Optional)</Label>
                      <Input
                        type="number"
                        step="0.01"
                ...
                <Card className="bg-gray-800 border-gray-700">
                  <CardContent className="pt-6 space-y-4">
                    ...
                    <div className="flex gap-2">
                      {isEditing ? (
                        <>
                          <Button onClick={handleEditAccount} className="flex-1">Save</Button>
